...
public class AIMLHandlerTest extends TestCase {
	...
	public void testCondition() throws Exception {
		...
		handler.characters(text = toCharArray("Conditioned output 1"), 0,
				text.length);
		...
		Condition expected = new Condition(null, null, new Li("condition1",
				"test value1", "Conditioned output 1"), new Li(null, null,
				"Else output"));
		assertEquals(expected, actual);
		...
		expected = new Condition("condition1", null, new Li(null,
				"test value1", "Conditioned output 1"), new Li(null, null,
				"Else output"));
		assertEquals(expected, actual);
		...
		attributes.addAttribute(null, null, "value", "String", "test value1");
		handler.startElement(null, null, "condition", attributes);
		handler.characters(text = toCharArray("Conditioned output"), 0,
				text.length);
		handler.endElement(null, null, "condition");

		expected = new Condition("condition1", "test value1",
				"Conditioned output");
		actual = (Condition) stack.peek();
		assertEquals(expected, actual);
	}
	...
	public void testGender() throws Exception {
		...
		handler.characters(text = toCharArray("I WILL DO WHAT HE SAYS"), 0,
				text.length);
	...
	public void testGossip() throws Exception {
		...
		handler.characters(
				text = toCharArray("write this input to alternate channel"), 0,
				text.length);
	...
	public void testLi() throws Exception {
		...
		handler.startElement(null, null, "li", attributes);
		handler.characters(
				text = toCharArray("defaultListItem type li element."), 0,
				text.length);
		handler.endElement(null, null, "li");
		...
		handler.startElement(null, null, "li", attributes);
		handler.characters(
				text = toCharArray("valueOnlyListItem type li element."), 0,
				text.length);
		handler.endElement(null, null, "li");

		expected = new Li(null, "test value",
				"valueOnlyListItem type li element.");
		...
		expected = new Li("condition", "test value",
				"valueOnlyListItem type li element.");
		actual = (Li) stack.peek();
		assertEquals(expected, actual);
	}
	...
	public void testLowercase() throws Exception {
		...
		handler.characters(text = toCharArray("CONVERT THIS TO LOWERCASE"), 0,
				text.length);
	...
	public void testPerson2() throws Exception {
		...
		handler.characters(text = toCharArray("I WILL DO WHAT YOU SAY"), 0,
				text.length);
	...
	public void testRandom() throws Exception {
		...
		handler.startElement(null, null, "li", attributes);
		handler.characters(
				text = toCharArray("defaultListItem type li element 1."), 0,
				text.length);
		handler.endElement(null, null, "li");
		handler.startElement(null, null, "li", attributes);
		handler.characters(
				text = toCharArray("defaultListItem type li element 2."), 0,
				text.length);
		handler.endElement(null, null, "li");
		handler.startElement(null, null, "li", attributes);
		handler.characters(
				text = toCharArray("defaultListItem type li element 3."), 0,
				text.length);
		handler.endElement(null, null, "li");
		handler.endElement(null, null, "random");

		Random expected = new Random(new Li(null, null,
				"defaultListItem type li element 1."), new Li(null, null,
				"defaultListItem type li element 2."), new Li(null, null,
				"defaultListItem type li element 3."));
	...
	public void testSentence() throws Exception {
		...
		handler.characters(
				text = toCharArray("testing sentence... will this work? hope so! let's see."),
				0, text.length);
		handler.endElement(null, null, "sentence");

		Sentence expected = new Sentence(
				"testing sentence... will this work? hope so! let's see.");
		Sentence actual = (Sentence) stack.peek();
		assertEquals(expected, actual);

		assertEquals("Testing sentence... Will this work? Hope so! Let's see.",
				actual.process(null));
	}
	...
	public void testTemplatePreserveWhitespace() throws Exception {
		char[] text;
		AttributesImpl attributes = new AttributesImpl();
		attributes.addAttribute(null, "space", "xml:space", "String",
				"preserve");

		handler.startElement(null, null, "template", attributes);
		handler.characters(
				text = toCharArray("   Hello unknown person,\nnice to meet you."),
				0, text.length);
		handler.endElement(null, null, "template");

		Template tag = (Template) stack.peek();
		assertEquals(
				new Template("   Hello unknown person,\nnice to meet you."),
				tag);
	}
	...
	public void testTopic() throws Exception {
		...
		Topic expected = new Topic("TEST TOPIC", new Category(new Pattern(
				"HELLO ALICE I AM *"), new Template("Hello ", new Star(1),
				", nice to meet you.")));
		assertEquals(expected, actual);
	...
	public void testUppercase() throws Exception {
		...
		handler.characters(text = toCharArray("Convert this to uppercase"), 0,
				text.length);
