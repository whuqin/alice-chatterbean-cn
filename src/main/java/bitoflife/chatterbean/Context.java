...
public class Context {
	...
	private final List<Request> requests = new LinkedList<Request>();// 这个列表是用来存放在聊天过程中用户的输入，可能会被重新用到。
	private final List<Response> responses = new LinkedList<Response>();

	private final Random random = new Random();
	private long seed = 0; // 这个参数是干什么用的？
	...
	public OutputStream outputStream() throws IOException {
		if (output == null) {
			...
			if (file.isDirectory())
				path = file.getPath() + "/gossip-" + id() + ".txt";// 这里的id()是干什么的
